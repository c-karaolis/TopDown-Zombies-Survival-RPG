...
namespace Pinwheel.Griffin
{
    ...
    public class GStylizedTerrain : MonoBehaviour
    {
            ...
            int lod)
        {
            ...
            GTextureNativeDataDescriptor<Color32> maskMap = new GTextureNativeDataDescriptor<Color32>((smoothNormal && useSmoothNormalMask) ? TerrainData.Mask.MaskMap : null);
            ...
            GTextureNativeDataDescriptor<Color32> albedoMap = new GTextureNativeDataDescriptor<Color32>(albedoToVertexColor != GAlbedoToVertexColorMode.None ? TerrainData.Shading.AlbedoMap : null);
            for (int i = 0; i < chunksToUpdate.Count; ++i)
            {
        ...
        public void UpdateTreesPosition(bool showProgressBar = false)
        {
            ...
            GTerrainChunk[] chunks = GetChunks();
            Rect[] chunkRects = new Rect[chunks.Length];
            for (int i = 0; i < chunks.Length; ++i)
            {
                chunkRects[i] = chunks[i].GetUvRange();
            }
            ...
            RaycastHit hitWorld;
            GTerrainChunk chunkToRaycast = null;
            ...
            for (int i = 0; i < treeCount; ++i)
            {
                ...
                for (int r = 0; r < dirtyRect.Length; ++r)
                {
                    ...
                    if (dirtyRect[r].Contains(uv))
                    {
                        ...
                        {
                            if (chunkRects[c].Contains(uv))
                            {
                                chunkToRaycast = chunks[c];
                            }
                        }

                        if (chunkToRaycast == null)
                            break;
                        ...
                        {
                            isHit = chunkToRaycast.Raycast(ray, out hit, float.MaxValue);
                        }
                        else
                        {
                            bool isHitTerrain = chunkToRaycast.Raycast(ray, out hitTerrain, float.MaxValue);
        ...
        public void UpdateGrassPatches(int prototypeIndex = -1, bool showProgressBar = false)
        {
...
#if UNITY_EDITOR
            string title = "Working";
            string info = "Updating Grass Transforms ... 0%";
            if (showProgressBar)
                GCommonGUI.CancelableProgressBar(title, info, 0);
#endif
            ...
            for (int i = 0; i < toUpdatePatches.Count; ++i)
            {
                ...
                if (showProgressBar)
                {
                    ...
                    int percent = Mathf.RoundToInt(progress * 100);
                    info = string.Format("Updating Grass Transforms ... {0}%", percent);
                    GCommonGUI.CancelableProgressBar(title, info, progress);
                }
#endif
            }
        ...
        private void UpdateGrassPatch(GGrassPatch patch, int prototypeIndex)
        {
            ...
            GTerrainChunk[] chunks = GetChunks();
            List<GTerrainChunk> overlapChunks = new List<GTerrainChunk>();
            List<Rect> overlapRects = new List<Rect>();
            Rect uvRange = patch.GetUvRange();
            for (int i = 0; i < chunks.Length; ++i)
            {
                Rect r = chunks[i].GetUvRange();
                if (uvRange.Overlaps(r))
                {
                    overlapChunks.Add(chunks[i]);
                    overlapRects.Add(r);
                }
            }
            ...
            for (int i = 0; i < instanceCount; ++i)
            {
                ...
                for (int r = 0; r < overlapChunks.Count; ++r)
                {
                    if (overlapRects[r].Contains(uv))
                    {
                        overlapChunkIndex = r;
                        break;
                    }
                }
                chunkToRaycast = overlapChunks[overlapChunkIndex];
