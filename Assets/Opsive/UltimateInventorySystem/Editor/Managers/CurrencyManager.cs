...
namespace Opsive.UltimateInventorySystem.Editor.Managers
{
    ...
    public class CurrencyManager : InventorySystemObjectBaseManager<Currency>
    {
        ...
        {
            for (int i = 0; i < m_MainManagerWindow.Database.Currencies.Length; ++i) {
                if (m_MainManagerWindow.Database.Currencies[i].name == name) {
                    return false;
                }
            }
            return true;
        ...
        {
            return m_MainManagerWindow.Database?.Currencies;
        }
        ...
        public override void OnRemove(int index)
        {
            ...
            if (EditorUtility.DisplayDialog("Remove Currency?", warningMessage, "Yes", "No")) {
                CurrencyEditorUtility.RemoveCurrency(SelectedObject, m_MainManagerWindow.Database);
                OnSelected(-1);
            }
        }
    ...
    public class CurrencyFamilyItemView : VisualElement
    {
        ...
        {
            AddToClassList(ManagerStyles.s_CurrencyFamilyItemView);

            var leftSideContainer = new VisualElement();
            leftSideContainer.AddToClassList(ManagerStyles.s_CurrencyFamilyItemView_LeftSide);
            ...
            rightSideContainer.AddToClassList(ManagerStyles.s_CurrencyFamilyItemView_RightSide);
