...
namespace Opsive.UltimateInventorySystem.Editor.Managers
{
    ...
    /// </summary>
    [OrderedEditorItem("Crafting Categories", 4)]
    [RequireDatabase]
    [System.Serializable]
    public class CraftingCategoryManager : InventorySystemObjectBaseManager<CraftingCategory>
    {
        ...
        public override void BuildVisualElements()
        {
            ...
            var craftingCategoryParentPicker = new CraftingCategoryField("", m_MainManagerWindow.Database,
            ...
            m_ParentCategoriesReorderableList = new ReorderableList(null, (VisualElement parent, int index) =>
                ...
                () =>
                {
                    craftingCategoryParentPicker.OpenPopUpWindow(
        ...
        {
            for (int i = 0; i < m_MainManagerWindow.Database.CraftingCategories.Length; ++i) {
                if (m_MainManagerWindow.Database.CraftingCategories[i].name == name) {
                    return false;
                }
            }
            return true;
        ...
        {
            var categories = m_MainManagerWindow.Database?.CraftingCategories;
            if (categories == null) {
                return;
            }
        ...
        {
            return m_MainManagerWindow.Database?.CraftingCategories;
        }
        ...
        {
            return CraftingCategoryEditorUtility.AddCraftingCategory(name, m_MainManagerWindow.Database, m_MainManagerWindow.GetDatabaseDirectory());
        }
        ...
        public override void OnRemove(int index)
        {
            ...
            if (removeCategory) {
                CraftingCategoryEditorUtility.RemoveCraftingCategory(SelectedObject, m_MainManagerWindow.Database);
            }
        }
